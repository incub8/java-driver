...
public class Mapper<T> {
    ...
    Mapper(MappingManager manager, Class<T> entityClass) {
    ...
    ListenableFuture<PreparedStatement> getPreparedQueryAsync(QueryType type, Set<AliasedMappedProperty> columns, EnumMap<Option.Type, Option> options) {
        ...
        if (existingFuture == null) {
            ...
            if (old != null) {
                ...
                TableMetadata tableMetadata = keyspace == null ? null : keyspace.getTable(entityMapper.table);

                final String queryString = type.makePreparedQueryString(tableMetadata, entityMapper, manager, columns, options.values());
    ...
    private ListenableFuture<BoundStatement> saveQueryAsync(T entity, final EnumMap<Option.Type, Option> options) {
        ...
        final EntityMapper<T> entityMapper = getEntityMapper();
        for (AliasedMappedProperty col : entityMapper.allColumns) {
        ...
        return Futures.transform(getPreparedQueryAsync(QueryType.SAVE, columnToValue.keySet(), options), new Function<PreparedStatement, BoundStatement>() {
            ...
            public BoundStatement apply(PreparedStatement input) {
                ...
                if (entityMapper.writeConsistency != null)
                    bs.setConsistencyLevel(entityMapper.writeConsistency);
    ...
    private ListenableFuture<BoundStatement> getQueryAsync(final List<Object> primaryKeys, final EnumMap<Option.Type, Option> options) {
        ...
        if (primaryKeys.size() != entityMapper.primaryKeySize())
            throw new IllegalArgumentException(String.format("Invalid number of PRIMARY KEY columns provided, %d expected but got %d", entityMapper.primaryKeySize(), primaryKeys.size()));
        ...
        return Futures.transform(getPreparedQueryAsync(QueryType.GET, options), new Function<PreparedStatement, BoundStatement>() {
            ...
            public BoundStatement apply(PreparedStatement input) {
                ...
                for (Object value : primaryKeys) {
                    AliasedMappedProperty column = entityMapper.getPrimaryKeyColumn(i);
                    if (value == null) {
                ...
                if (entityMapper.readConsistency != null)
                    bs.setConsistencyLevel(entityMapper.readConsistency);
    ...
    private ListenableFuture<BoundStatement> deleteQueryAsync(T entity, EnumMap<Option.Type, Option> options) {
        ...
        for (int i = 0; i < entityMapper.primaryKeySize(); i++) {
            pks.add(entityMapper.getPrimaryKeyColumn(i).mappedProperty.getValue(entity));
        }
    ...
    private ListenableFuture<BoundStatement> deleteQueryAsync(final List<Object> primaryKey, final EnumMap<Option.Type, Option> options) {
        ...
        if (primaryKey.size() != entityMapper.primaryKeySize())
            throw new IllegalArgumentException(String.format("Invalid number of PRIMARY KEY columns provided, %d expected but got %d", entityMapper.primaryKeySize(), primaryKey.size()));
        ...
        return Futures.transform(getPreparedQueryAsync(QueryType.DEL, options), new Function<PreparedStatement, BoundStatement>() {
            ...
            public BoundStatement apply(PreparedStatement input) {
                ...
                for (Object value : primaryKey) {
                    @SuppressWarnings("unchecked")
                    AliasedMappedProperty column = entityMapper.getPrimaryKeyColumn(columnNumber);
                    if (value == null) {
    ...
    public Result<T> map(ResultSet resultSet) {
        ...
        return new Result<T>(resultSet, getEntityMapper(), useAlias);
    }
    ...
    public Result<T> mapAliased(ResultSet resultSet) {
        return (manager.protocolVersionAsInt == 1)
                ...
                : new Result<T>(resultSet, getEntityMapper(), true);
    }
